...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train direct posterior-mean denoiser f(x, level).")
    parser.add_argument("--snapshots", required=True, type=Path)
    parser.add_argument("--output", default=Path("checkpoints/denoiser.pt"), type=Path)
    ...
    parser.add_argument("--level-min", type=float, default=0.01)
    parser.add_argument("--level-max", type=float, default=2.5)
    parser.add_argument(
        "--sample-kind",
    ...
    parser.add_argument(
        ...
        default=True,
        help="Train the odd posterior mean 0.5 * (f(x, level) - f(-x, level)).",
    )
...
def main() -> None:
    ...
    model = DilatedConvNet(length=length, hidden_channels=args.hidden_channels, output_activation="tanh")
